...
`circuit_types.py` contains the Circuits class, which takes in a truth table and encodes it as a logic gate. To get the truth table, we wrote a parser in `blif_parser.py`. The core of Fujita (2012) relies on CEGIS to do the iterative synthesis, so we needed to encode our circuit as an SMT problem (`encoder.py`). We used Z3 as the solver, which we finally do in `cegis.py`. 

### Experiment

We changed one net, `t3`, in `example2.blif`. Here is the result of a sample run:
...
We can see that the rectifier works as intended in the circumstances we have tested. Further, we have noted that the number of iterations is small compared to the search space.
